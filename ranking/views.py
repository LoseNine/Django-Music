...
def rankingView(request):
    #热搜歌曲
    searching_song=Dynamic.objects.select_related('song').order_by('-dynamic_search').all()[:4]
    ...
    if song_type:
        song_info=Dynamic.objects.select_related('song')\
            .filter(song__song_type=song_type)\
            .order_by('-dynamic_plays')\
            .all()[:10]
    else:
        song_info=Dynamic.objects.select_related('song')\
        .order_by('-dynamic_plays')\
